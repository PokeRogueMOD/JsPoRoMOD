...
# Simulate full speciesStarterCosts raw string input (normally this would be read from a file)
# For this example, we only use a partial sample to build a working version
# Let's extract all key-value pairs from the speciesStarterCosts-style raw text
...
"""  # Replace this string with the full raw JS object for complete extraction

# Extract the static mapping from the raw string
matches = re.findall(r"\[Species\.([A-Z0-9_]+)\]:\s*(\d+),", raw_input)
static_js_lines = ['export const speciesStarterCostsStatic = {']
for name, cost in matches:
    static_js_lines.append(f'  "{name}": {cost},')
static_js_lines.append('};')

# Output full list in multiple blocks if necessary
static_js_full = "\n".join(static_js_lines)
print(static_js_full)